...
shutil.copy2(template,
             f"{current_directory}/settings/dynamicsettings_in{inst}.txt")  # This works
# now load the copy and edit it
fileout = open(f"{current_directory}/settings/dynamicsettings_in{inst}.txt", 'w')
for line in filein:
    fileout.write(line.replace("Positive control extinction network n6 L15 N4.0 I1000 in0", f"Dynamic_Extinction_Network_in{inst}").replace(
        "inputDirectory = /Users/james/",
        f"inputDirectory = {current_directory}".replace(
        "observationsFile = Extinction Network Positive Control extinction network with n6 L15 N4 I1000 in0.txt",
        f"observationsFile = {input}".replace(
        "outputDirectory = Users/james",
        f"outputDirectory = {current_directory}/BanjoOutputs".replace(
        "reportFile = static.report.txt", f"reportFile = dynamic.in{inst}Report.txt")))))
fileout.close()
filein.close()