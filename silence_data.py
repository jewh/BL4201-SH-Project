...
def get_columns(array):
    ni = np.shape(array)[1]
    print(f"There are {ni} many columns in the input")
    nj = np.shape(array)[0]
    columns = []
    for i in range(0, ni):
        columni = []
...
# Want the seed to change with each run, so change it to the instance each run
inst = int(between(files[0], " in", ".txt"))
print(f"This is the {inst} instance")
# Now specify the seed
rd.seed(inst) # Note that seed = 2 wouldn't produce a dataset with 6 nodes, only 5 - thus set to 13 for instance 2
# This allows reproducibility whilst also variations between instances
# Now load in the structure
network_structure = np.loadtxt(f"C:/Users/User/Documents/GitHub/BL4201-SH-Project/genereg/re-run/structures/gene network structure static network with n6 L15 N10 I1000 in{inst}.txt")
# Now get the nodes
nodes = get_nodes(network_structure, 6)
