...
# Assume each file in the directory is the same type

type = ''
base = files[0]

# Specify which file types to look for

types = ["Output", "Positive", "Neg"]

# Look for them

for item in types:
    if str(base).find(item) >= 0:
        type += item
        break
...
shutil.copy2(template,
             f"{current_directory}/settings/allfiledynamicsettings{type}_in{inst}.txt")
# This works
# now load the copy and edit it
fileout = open(f"{current_directory}/settings/allfiledynamicsettings{type}_in{inst}.txt", 'w+')
for line in filein:
